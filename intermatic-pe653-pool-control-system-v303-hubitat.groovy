 ...
 */
metadata {
  definition(name: "Intermatic PE653 Pool Control System v3.03", author: "KeithR26", namespace: "KeithR26") {
    capability "Actuator"
    capability "Switch"
    capability "Polling"
...
def getDELAY() {
  ZWdelay
} // How long to delay between commands to device (configured)
def getMIN_DELAY() {
  "800"
...
def getPOOL_SPA_SCHED_PARAM() {
  21
} // Pool/Spa mode Schedule #3 - 0x15
def getPOOL_SPA_CHAN() {
  39
} // Pool/Spa channel - 0x27
def getPOOL_SPA_EP() {
  6
} // Pool/Spa endpoint - 6
def getVSP_SCHED_NO(int spd) {
  (35 + (spd * 3))
} // VSP Speed 1 Schedule #3 - 0x26
def getVSP_SPEED(int sched) {
  ((sched - 35) / 3)
} // Convert from sched to speed
def getVSP_CHAN_NO(int spd) {
  (16 + (spd - 1))
} // VSP Speed 1 Channel  - 0x10 - 0x13
def getVSP_EP(int spd) {
  (6 + spd)
} // VSP Endpoint 7 - 10
def getVSP_SPEED_FROM_CHAN(int chan) {
  ((chan - 16) + 1)
} // Convert from channel to speed - 0x10
def getVSP_ENABLED() {
  (operationMode2 >= "4") ? 1: 0
} // True if a Variable Speed Pump Configured
def getPOOL_SPA_COMBO() {
  (poolSpa1 == "2") ? 1: 0
} // True if both Pool and Spa
  ...
  if (description.startsWith("Err")) {
    ...
    try {
      ...
      if (command.contains("9100")) {
        ...
        def cmd = zwave.parse(description, [0x20: 1, 0x25: 1, 0x27: 1, 0x31: 1, 0x43: 1, 0x60: 1, 0x70: 2, 0x72: 1, 0x81: 1, 0x85: 2, 0x86: 1, 0x73: 1, 0x91: 1])
        if (cmd) {
          if (debugLevel > "0") {
...
def zwaveEvent(hubitat.zwave.commands.clockv1.ClockReport cmd) {
  def time1 = ""
  time1 = "${String.format(" % 02 d ",cmd.hour)}:${String.format(" % 02 d ",cmd.minute)}"
...
def zwaveEventManufacturerProprietary(byte[] payload, payloadStr) {
  if (debugLevel > "0") {
    log.debug "ManufacturerProprietary event, [1]:${String.format(" % 02 X ",payload[1])}  [4]:${String.format(" % 02 X ",payload[4])}  payload: ${payloadStr}"
  }
  def rslt = []
  ...
  for (def i = 0; i < payload.length; i++) {
    oldP += " ${String.format(" % 02 X ",oldResp[i])}"
    newP += " ${String.format(" % 02 X ",payload[i])}"
    oldD += " ${String.format(" % 03 d ",oldResp[i])}"
    newD += " ${String.format(" % 03 d ",payload[i])}"
...
def getCLOCK_MINUTE_84() {
  16
} // Clock Minute
def getCLOCK_HOUR_84() {
  15
} // Clock Hour
def getWATER_TEMP_84() {
  12
} // Water Temperature
def getAIR_TEMP_FREEZE_84() {
  13
} // Air Temperature for Freeze sensing
def getAIR_TEMP_SOLAR_84() {
  14
} // Air Temperature for Solar control
def getSWITCHES_84() {
  8
} // Bit mask of 5 switches. SW1 = 01X, SW5 = 10X
def getPOOL_SPA_MODE_84() {
  11
} // Pool/Spa mode. 01x Pool mode, 00x Spa mode
def getVSP_SPEED_84() {
  20
} // VSP Speed bit mask. 01x = VSP1, 08x = VSP4
...
def process84Event(byte[] payload) {
  ...
  //	Update Clock
  def time1 = "${String.format(" % 02 d ",payload[CLOCK_HOUR_84])}:${String.format(" % 02 d ",payload[CLOCK_MINUTE_84])}"
  //	log.debug "from PE653: ${time1}"
  //	state.VersionInfo = "ClockReport: ${time1}"
...
def getHEATER_87() {
  15
} // Heater. 04x = on, 00x = off
def getCLOCK_MINUTE_87() {
  25
} // Clock Minute
def getCLOCK_HOUR_87() {
  24
} // Clock Hour
...
private List createMultipleEvents(Integer endpoint, Integer externalParm, String myParm) {
  ...
  def devObj = getChildDevices() ?.find {
...
def List setClock() {
  ...
  def time2 = "${String.format(" % 02 d ",nowCal.get(Calendar.HOUR_OF_DAY))}:${String.format(" % 02 d ",nowCal.get(Calendar.MINUTE))}"
...
def setMode(int mode) {
  ...
  switch (mode) {
    case 1:
      MxSw = [M1Sw1, M1Sw2, M1Sw3, M1Sw4, M1Sw5];
      MxMode = M1Mode;
      MxTemp = M1Temp;
      MxVSP = M1VSP;
      break
    case 2:
      MxSw = [M2Sw1, M2Sw2, M2Sw3, M2Sw4, M2Sw5];
      MxMode = M2Mode;
      MxTemp = M2Temp;
      MxVSP = M2VSP;
      break
    case 3:
      MxSw = [M3Sw1, M3Sw2, M3Sw3, M3Sw4, M3Sw5];
      MxMode = M3Mode;
      MxTemp = M3Temp;
      MxVSP = M3VSP;
      break
    case 4:
      MxSw = [M4Sw1, M4Sw2, M4Sw3, M4Sw4, M4Sw5];
      MxMode = M4Mode;
      MxTemp = M4Temp;
      MxVSP = M4VSP;
      break
...
def List on1() {
  delayBetweenLog(addRefreshCmds(setChanState(1, 0xFF)))
}
def List on2() {
  delayBetweenLog(addRefreshCmds(setChanState(2, 0xFF)))
}
def List on3() {
  delayBetweenLog(addRefreshCmds(setChanState(3, 0xFF)))
}
def List on4() {
  delayBetweenLog(addRefreshCmds(setChanState(4, 0xFF)))
}
def List on5() {
  delayBetweenLog(addRefreshCmds(setChanState(5, 0xFF)))
}
def List off1() {
  delayBetweenLog(addRefreshCmds(setChanState(1, 0)))
}
def List off2() {
  delayBetweenLog(addRefreshCmds(setChanState(2, 0)))
}
def List off3() {
  delayBetweenLog(addRefreshCmds(setChanState(3, 0)))
}
def List off4() {
  delayBetweenLog(addRefreshCmds(setChanState(4, 0)))
}
def List off5() {
  delayBetweenLog(addRefreshCmds(setChanState(5, 0)))
}

// May be called by CoRE
def List setVSPSpeed(sp) {
  delayBetweenLog(addRefreshCmds(setVSPSpeedInternal(sp)))
}
// Called by switch presses on the VSP buttons.
def List setVSPSpeed0() {
  delayBetweenLog(addRefreshCmds(setVSPSpeedInternal(0)))
}
def List setVSPSpeed1() {
  delayBetweenLog(addRefreshCmds(setVSPSpeedInternal(1)))
}
def List setVSPSpeed2() {
  delayBetweenLog(addRefreshCmds(setVSPSpeedInternal(2)))
}
def List setVSPSpeed3() {
  delayBetweenLog(addRefreshCmds(setVSPSpeedInternal(3)))
}
def List setVSPSpeed4() {
  delayBetweenLog(addRefreshCmds(setVSPSpeedInternal(4)))
}
def List setMode1() {
  delayBetweenLog(addRefreshCmds(setMode(1)))
}
def List setMode2() {
  delayBetweenLog(addRefreshCmds(setMode(2)))
}
def List setMode3() {
  delayBetweenLog(addRefreshCmds(setMode(3)))
}
def List setMode4() {
  delayBetweenLog(addRefreshCmds(setMode(4)))
}

def List setSpaMode() {
  delayBetweenLog(addRefreshCmds(setSpaModeInternal()))
}
def List setPoolMode() {
  delayBetweenLog(addRefreshCmds(setPoolModeInternal()))
}
def List togglePoolSpaMode() {
  delayBetweenLog(addRefreshCmds(togglePoolSpaModeInternal()))
}

def List quickSetSpa(degrees) {
  delayBetweenLog(addRefreshCmds(setSpaSetpointInternal("${degrees}".toDouble())))
}
def List quickSetPool(degrees) {
